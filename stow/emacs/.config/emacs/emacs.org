...
Emacs does not inherit shell PATH when launched from desktop environment.
This fixes stuff like lsp not being found. Uses fish with interactive flag for nvm support.
#+begin_src emacs-lisp
...
Use .editorconfig files to maintain consistent coding styles across editors
